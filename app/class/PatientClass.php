...
class PatientClass extends PdoDatabse {
    ...
    */
    ...
    public function updateProfile($data)
    {
        ...
        if (true) {
            $stmt = $this->pdo->prepare('UPDATE persons SET first_name = ?, last_name = ?, personal_email = ?, phone = ?, country = ?, city = ?, state = ? , address = ?, zip = ? WHERE id = ?');
            try {
                if ($stmt->execute([
                    $data['fname'],
                    $data['lname'],
    ...
    public function general()
    {
        ...
        $user['orders'] = $this->order->allNew($id, 'patient');
        
        $user['notice'] = $this->notice->allNew($id,'patient');
    ...
    */
    public function registration($email,$fname,$lname,$pass){
    	...
    	if($stmt->execute([$fname,$lname,$email,$pass,$confCode,'patient', 1])){
