...
class DoctorClass extends PdoDatabse {
    ...
    private $fillable = ['name','author','address', 'phone','email','job_title','gender','dob','hospital_id'];
    ...
    */
    public function registration($email,$fname,$lname,$pass){
    	...
    	if($stmt->execute([$fname,$lname,$email,$pass,$confCode,'doctor',1])){
    ...
    */
    ...
    public function updateProfile($data)
    {
        ...
        if (true) {
            $stmt = $this->pdo->prepare('UPDATE doctors SET name = ?, email = ?, phone = ?,  address = ?, chamber = ? WHERE id = ?');
            try {
                if ($stmt->execute([
                    $data['name'],
                    $data['email'],
    ...
    public function general()
    {
        ...
        $user['orders'] = $this->order->allNew($id, 'doctor');
        
		$user['notice'] = $this->notice->allNew($id, 'doctor');
    ...
    */
    ...
    public function getPersonalInfo(int $id){
        if (is_null($this->pdo)) {
            ...
            $pdo = $this->pdo;
            $stmt = $pdo->prepare('SELECT * FROM users u,doctors p WHERE u.id = ? and u.id = p.user_id');
            $stmt->execute([$id]);
            $user = $stmt->fetch();
